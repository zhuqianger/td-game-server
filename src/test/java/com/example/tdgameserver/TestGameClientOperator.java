...
@Slf4j
public class TestGameClientOperator {
    ...
    public TestGameClientOperator(String host, int port, Long playerId){
        this.host = host;
        this.port = port;
    ...
    public TestGameClientOperator(String host, int port, String username, String password){
        this.host = host;
        this.port = port;
    ...
    private void parseAndSendCommand(String input) {
        ...
        switch (command) {
            ...
            case "6":
                if (data.isEmpty()) {
                    System.out.println("用法: custom <message_id> <json_data>");
                    return;
                }
                String[] customParts = data.split("\\s+", 2);
                if (customParts.length != 2) {
                    System.out.println("用法: custom <message_id> <json_data>");
                    return;
                }
                try {
                    int messageId = Integer.parseInt(customParts[0]);
                    String jsonData = customParts[1];
                    sendCustomMessage(messageId, jsonData);
                } catch (NumberFormatException e) {
                    System.out.println("消息ID必须是数字");
                }
                break;
            default:
     ...
     * 发送获取干员消息
     */
    private void sendGetOperatorsMessage() {
        GameMessage message = new GameMessage(MessageId.REQ_GET_PLAYER_OPERATORS.getId(), "{}".getBytes());
     ...
     * 发送自定义消息
     */
    private void sendCustomMessage(int messageId, String jsonData) {
        GameMessage message = new GameMessage(messageId, jsonData.getBytes());
        channel.writeAndFlush(message);
        log.info("发送自定义消息 - 消息ID: {}, 数据: {}", messageId, jsonData);
    }
    ...
    private void printHelp() {
        ...
        System.out.println("get_operators            - 获取玩家干员");
        ...
        System.out.println("custom <msg_id> <json>   - 发送自定义消息");
    ...
    public static void main(String[] args) throws Exception{
        ...
        TestGameClientOperator client1 = new TestGameClientOperator("localhost", 8888, 1L);  // 用户名: 1, 密码: 123456
